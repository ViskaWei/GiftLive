...
ROOT_DIR = Path(sys.argv[2]) if len(sys.argv) > 2 else Path("/home/swei20/GiftLive")
...
def render_markdown(content, base_path):
    ...
    # Pattern: <img src="../img/xxx.png"> or <img src="img/xxx.png">
    def fix_img_path(match):
        ...
        if src_match:
            ...
            # base_path is relative to ROOT_DIR (e.g., "gift_allocation/exp/exp_kuailive_eda_20260108.md")
            if base_path == Path('.'):
                base_dir = Path('.')
            else:
            ...
            base_abs = ROOT_DIR / base_dir if base_dir != Path('.') else ROOT_DIR
            resolved_path = (base_abs / rel_path).resolve()
            
            # Make path relative to ROOT_DIR
            try:
                rel_to_root = resolved_path.relative_to(ROOT_DIR)
                # Convert to URL path
                url_path = '/' + str(rel_to_root).replace('\\', '/')
                # Replace the src attribute
                new_img_tag = re.sub(r'src=["\'][^"\']+["\']', f'src="{url_path}"', img_tag)
                return new_img_tag
            except ValueError:
                # Path is outside ROOT_DIR, keep original
                return img_tag
        return img_tag
    
    # Replace all img tags
    html = re.sub(r'<img[^>]+>', fix_img_path, html)
        ...
        try:
            ...
            if file_path.suffix.lower() == '.md' and file_path.exists():
                ...
                with open(file_path, 'r', encoding='utf-8') as f:
                    content = f.read()
                html_body = render_markdown(content, rel_path)
            ...
            elif file_path.suffix.lower() in IMAGE_EXTENSIONS and file_path.exists():
                # Serve image files
                ...
                body = f'<h1>📁 {rel_path if rel_path != Path(".") else "GiftLive"}</h1><ul class="dir-listing">{"".join(links)}</ul>'
