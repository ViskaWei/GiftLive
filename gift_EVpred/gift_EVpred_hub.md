...
- 🔴 **P0**：重构特征工程（MVP-1.2）→ 探索序列/实时/内容特征，目标 Top-1% > 30%
- 🔴 **P0**：任务降级验证（MVP-1.3）→ 验证二分类 P(gift>0) 是否可行，若 AUC > 0.7 则用于召回
...
│   ├── Q1.6: 新特征是否能提升预测力？ → 🔆 MVP-1.2 探索中
│   │   ├── H2.1: 序列特征有提升 → 最近 N 次打赏金额/间隔
│   │   ├── H2.2: 实时上下文有提升 → watch_time, session 互动
│   │   ├── H2.3: 内容匹配度有提升 → user 偏好 vs streamer 内容
│   │   └── H2.4: 组合特征有叠加效果
│   └── Q1.7: 二分类任务是否可行？ → 🔆 MVP-1.3 验证中
