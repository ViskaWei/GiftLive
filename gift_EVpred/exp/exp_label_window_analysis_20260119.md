...
- [x] 对比不同窗口对模型性能的影响（见 §5）

---

## 5. Ridge Baseline 对比实验

使用 Ridge Regression (alpha=1.0) 对比 1min vs 60min 窗口的模型性能。

### 5.1 实验结果

| 指标 | 1min 窗口 | 60min 窗口 | 差异 |
|------|-----------|------------|------|
| Gift Rate (Test) | 1.54% | 1.55% | +0.02pp |
| Total Revenue (Test) | 1,758,581 | 1,809,367 | +2.8% |
| **RevCap@1%** | **51.4%** | **51.0%** | **+0.4pp** |
| RevCap@0.5% | 43.3% | 42.9% | +0.4pp |
| RevCap@2% | 56.3% | 55.9% | +0.4pp |
| Whale Recall@1% | 34.6% | 34.6% | 0pp |
| Whale Precision@1% | 5.0% | 5.1% | -0.1pp |
| **CV (稳定性)** | **9.4%** | **10.4%** | **-0.9pp** |

### 5.2 结论

1. **RevCap@1% 几乎相同**：1min (51.4%) vs 60min (51.0%)，差异仅 0.4pp
2. **1min 窗口稳定性更好**：CV 9.4% < 10.4%，首次低于 10% 阈值
3. **延迟打赏对预测无贡献**：虽然 60min 多 2.8% 金额，但模型性能没有提升

### 5.3 解释

- 98.2% 的打赏是即时行为（Δt=0），这些包含了所有有预测价值的信号
- 延迟打赏（1.8%）更多是噪声，去掉反而让模型更稳定
- **推荐使用 1 分钟窗口作为默认配置**
