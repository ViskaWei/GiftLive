#!/usr/bin/env python3
"""
Fast Baseline: Optimized Frozen Features + Fast Config
======================================================

Experiment: Fast Baseline with 15min window + optimized frozen features

Key improvements:
1. Use optimized apply_frozen_features_optimized() (107x faster)
2. Use 15min label window (vs 1h) for faster iteration
3. Use fast training config (learning_rate=0.1, max_rounds=200)
4. Precompute and cache frozen lookups
...
def prepare_features(gift, user, streamer, room, click, click_base, train_df, lookups=None):
    ...
    # Create or use cached lookups
    if lookups is None:
        lookups, _, _ = create_frozen_features(gift, click, train_df)
...
def main():
    ...
    log_message("=" * 60)
    log_message("Fast Baseline: Optimized Frozen Features")
    log_message("=" * 60)
    log_message(f"Config: {CONFIG['label_window_hours']*60:.0f}min window, {CONFIG['feature_version']} features")
    ...
    if CONFIG['cache_lookups']:
        ...
        lookup_cache_path = FEATURES_DIR / f"frozen_lookups_{train_min_ts}_{train_max_ts}.pkl"
        ...
        if lookup_cache_path.exists():
            ...
            log_message("Computing frozen lookups...")
    ...
    train_df, _ = prepare_features(gift, user, streamer, room, click, click_train, click_train, lookups)
    
    log_message("Preparing val features...")
    val_df, _ = prepare_features(gift, user, streamer, room, click, click_val, click_train, lookups)
    
    log_message("Preparing test features...")
    test_df, _ = prepare_features(gift, user, streamer, room, click, click_test, click_train, lookups)
    ...
    # Save results
    results_path = RESULTS_DIR / f"baseline_fast_{datetime.now().strftime('%Y%m%d')}.json"
    with open(results_path, 'w') as f:
        json.dump(metrics, f, indent=2)
    log_message(f"Results saved to: {results_path}")
    
    # Save model
    model_path = MODELS_DIR / f"baseline_fast_{datetime.now().strftime('%Y%m%d')}.pkl"
