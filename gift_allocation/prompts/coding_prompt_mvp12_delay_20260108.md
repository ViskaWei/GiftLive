...
| 图号 | 图表类型 | X轴 | Y轴 | 保存路径 |
|------|---------|-----|-----|---------|
| Fig1 | histogram + fitted curve | Delay (minutes) | Density | `gift_allocation/img/delay_distribution.png` |
| Fig2 | grouped bar | Time Window | ECE | `gift_allocation/img/delay_ece_by_window.png` |
| Fig3 | line (3 models) | Predicted Prob | Actual Freq | `gift_allocation/img/delay_calibration_comparison.png` |
| Fig4 | grouped bar | Recency Window | PR-AUC | `gift_allocation/img/delay_prauc_by_recency.png` |
| Fig5 | bar | Model | ECE / Brier | `gift_allocation/img/delay_model_comparison.png` |
...
### 5.1 实验报告
- **路径**: `gift_allocation/exp/exp_delay_modeling_20260108.md`
- **模板**: 已存在草稿，补全 §4-§6
- **必须包含**:
  - ⚡ 核心结论速览（一句话 + 关键数字对比表）
  - 📊 实验图表（所有 5 张图 + 观察）
...
### 5.3 数值结果
- **格式**: JSON
- **路径**: `gift_allocation/results/delay_modeling_20260108.json`
- **内容**:
```yaml
required_fields:
...
| 目标文件 | 更新内容 | 章节 |
|---------|---------|------|
| `gift_allocation/gift_allocation_roadmap.md` | MVP-1.2 状态 → ✅; 结论快照 | §2.1, §4.3 |
| `gift_allocation/gift_allocation_hub.md` | DG2 验证状态; 更新 P4 原则 | §5 DG2, §6 |
