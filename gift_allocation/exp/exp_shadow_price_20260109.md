# 📗 实验报告：影子价格/供需匹配

> **Experiment ID:** `EXP-20260109-gift-allocation-16`  
> **MVP:** MVP-5.2  
> **Author:** Viska Wei  
> **Date:** 2026-01-09  
> **Status:** ✅ Completed  
> **Gate:** Gate-5B

---

## 🔗 上游追溯

- **Coding Prompt**: `gift_allocation/prompts/coding_prompt_mvp52_shadow_price_20260109.md`
- **Hub**: `../gift_EVpred_hub.md`
- **Roadmap**: `../gift_EVpred_roadmap.md`

---

## ⚡ 核心结论速览

### 一句话总结

**❌ Gate-5B FAIL**: 影子价格框架未能同时满足收益提升(+5%)和约束满足率(>90%)目标。最佳Shadow Price策略收益仅提升+2.74%，且容量约束满足率仅82.8%。

### 假设验证

| 假设 | 结果 | 关键数值 |
|------|------|----------|
| H5B-1: 影子价格能统一处理多约束 | ⚠️ 部分验证 | 成功实现5种约束的统一框架 |
| H5B-2: 收益 ≥ Greedy+5% | ❌ 失败 | 实际: +2.74% (目标: +5%) |
| H5B-3: 约束满足率 >90% | ❌ 失败 | 容量: 82.8%, 冷启动: 91.5% |

### 关键数字速查

| 指标 | Greedy | Greedy+Rules | Shadow Price Core | Shadow Price All |
|------|--------|--------------|-------------------|------------------|
| Revenue | 248,708 | 259,542 | 252,937 | 255,516 |
| Δ vs Greedy | 0% | **+4.36%** | +1.70% | +2.74% |
| Cold Start Rate | 90.1% | **97.8%** | 92.0% | 91.5% |
| Capacity Satisfy | N/A | N/A | 83.4% | 82.8% |
| Gini | 0.520 | 0.528 | 0.528 | 0.534 |

---

## 🎯 目标

### 实验目的
验证Primal-Dual影子价格框架能否：
1. 统一处理多种约束（容量、冷启动、头部占比、鲸鱼分散、频控）
2. 通过对偶变量自动学习约束价值
3. 在满足约束的同时提升收益

### Gate-5B 判定标准
- 收益 ≥ Greedy+5% **AND** 约束满足率 >90% → PASS
- 否则 → FAIL

### 预期结果
- 若 PASS → 替换Greedy+规则为统一框架
- 若 FAIL → 保留Greedy+软约束方案

---

## 🧪 实验设计

### 核心公式

**分配决策**:
$$s^* = \arg\max_s \left[ \widehat{EV}(u,s) - \sum_c \lambda_c \cdot \Delta g_c(u \to s) \right]$$

**对偶变量更新**:
$$\lambda_c^{(t+1)} = \left[ \lambda_c^{(t)} + \eta \cdot (g_c^{(t)} - b_c) \right]_+$$

### 约束类型

| 约束 | 描述 | λ_init |
|------|------|--------|
| C1: Capacity | 主播并发容量上限 | 0.1 |
| C2: Cold Start | 新主播最低曝光保障 | 0.5 |
| C3: Head Cap | Top-10%收益占比上限(≤50%) | 0.1 |
| C4: Whale Spread | 每主播鲸鱼数上限(≤2) | 0.2 |
| C5: Frequency | 用户-主播重复曝光限制(≤3) | 0.3 |

### 实验配置

```yaml
simulation:
  n_users: 10000
  n_streamers: 500
  n_rounds: 50
  users_per_round: 200
  n_simulations: 50
  seed: 42

policies:
  - greedy (baseline)
  - greedy_with_rules (cold_start_bonus + freq_penalty)
  - shadow_price_core (capacity + cold_start + head_cap)
  - shadow_price_all (all 5 constraints)
```

---

## 📊 实验图表

### Fig 1: Revenue Comparison

![Revenue Comparison](../img/mvp52_revenue_comparison.png)

**观察**:
- Greedy+Rules (+4.36%) 表现优于 Shadow Price All (+2.74%)
- Shadow Price 框架在收益上未能超越简单规则方案
- 两种Shadow Price策略收益差异不大 (1.0%)

### Fig 2: Constraint Satisfaction Heatmap

![Constraint Satisfaction](../img/mvp52_constraint_satisfaction.png)

**观察**:
- 冷启动成功率：Greedy+Rules最高(97.8%)，Shadow Price约91-92%
- 容量满足率：Shadow Price策略约82-83%，未达90%目标
- Top-10%占比：所有策略表现相似(42-45%)

### Fig 3: Lambda Convergence

![Lambda Convergence](../img/mvp52_lambda_convergence.png)

**观察**:
- Cold Start λ: 从0.5快速上涨至1.25并稳定收敛
- Head Cap λ: 从0.1逐步上涨至0.42并收敛
- Whale Spread λ: 持续缓慢上涨至0.55，收敛性较差
- Capacity λ: 保持0.1不变（约束未触发）
- Frequency λ: 缓慢上涨至0.36

### Fig 4: Learning Rate Sensitivity

![LR Sensitivity](../img/mvp52_lr_sensitivity.png)

**观察**:
- 收益随η增大而略微上升 (244k → 249k)
- 冷启动率对η不敏感 (91-92%)
- 稳定性随η增大而下降 (0.009 → 0.175)
- **推荐**: η=0.05 是收益与稳定性的较好平衡点

### Fig 5: Constraint Ablation

![Constraint Ablation](../img/mvp52_constraint_ablation.png)

**观察**:
- 添加约束普遍导致收益下降
- Capacity only: 基本与Greedy持平
- +Cold Start: -0.57%
- +Head Cap: -1.10%
- +Whale Spread / +Frequency: 无额外影响
- **结论**: 当前约束设计存在收益损失，需要重新校准

### Fig 6: Pareto Frontier

![Pareto Frontier](../img/mvp52_pareto_frontier.png)

**观察**:
- Greedy+Rules在帕累托前沿上最优（高收益 + 高约束满足）
- Shadow Price All虽有更多约束，但未能在收益-约束权衡上胜出
- Greedy基线虽收益低但Gini较低（更公平）

---

## 💡 关键洞见

### 宏观层

1. **简单规则 > 复杂框架**: Greedy+Rules的简单冷启动bonus+频控惩罚策略表现优于Shadow Price统一框架
2. **约束越多 ≠ 越好**: 添加更多约束导致收益持续下降，且额外约束（whale_spread, frequency）贡献边际
3. **容量约束未触发**: λ_capacity保持初始值0.1，说明当前模拟中容量约束不是瓶颈

### 模型层

1. **对偶变量收敛性**: cold_start和head_cap收敛良好，whale_spread收敛较慢
2. **学习率选择**: η=0.05-0.1是较好区间，过高导致不稳定
3. **约束惩罚设计**: 当前惩罚函数可能过于aggressive，导致收益损失

### 实验层

1. **数据规模效应**: 50次模拟的统计置信度足够，标准差在合理范围
2. **模拟器校准**: 冷启动成功率普遍高(90%+)可能与真实场景有差距

---

## 📝 结论

### Gate-5B 判定

**❌ FAIL**

| 判定项 | 要求 | 实际 | 结果 |
|--------|------|------|------|
| 收益提升 | ≥ Greedy+5% | +2.74% | ❌ |
| 冷启动成功率 | ≥ 30% | 91.5% | ✅ |
| 容量满足率 | ≥ 90% | 82.8% | ❌ |

### 决策建议

**保留 Greedy+Rules 方案**，暂不采用Shadow Price框架。

原因：
1. Greedy+Rules在收益上更优 (+4.36% vs +2.74%)
2. Greedy+Rules的冷启动效果更好 (97.8% vs 91.5%)
3. Shadow Price框架增加了复杂度但未带来明显收益

### 设计启示

| 编号 | 设计启示 | 来源 |
|------|---------|------|
| DG7-1 | 简单规则在现阶段足够有效，复杂框架需要更好的约束校准 | 政策对比 |
| DG7-2 | 冷启动bonus=0.5是有效参数，已在Greedy+Rules中验证 | 政策对比 |
| DG7-3 | 对偶变量更新学习率η=0.05是稳定性与收益的平衡点 | LR Sweep |
| DG7-4 | Whale Spread约束收敛性差，可能需要重新设计惩罚函数 | Lambda收敛 |

### 后续建议

1. **短期**: 继续使用Greedy+Rules，监控线上冷启动和收益指标
2. **中期**: 优化Shadow Price的约束惩罚函数设计，减少收益损失
3. **长期**: 考虑在线学习λ值，适应流量动态变化

---

## 📎 附录

### A. 数值结果表

#### A.1 Policy Comparison (n=50)

| Policy | Revenue (mean±std) | Gini | Cold Start | Top-10 Share | Capacity Satisfy |
|--------|-------------------|------|------------|--------------|------------------|
| greedy | 248,708 ± 22,991 | 0.520 | 90.1% | 42.7% | N/A |
| greedy_with_rules | 259,542 ± 38,488 | 0.528 | 97.8% | 44.7% | N/A |
| shadow_price_core | 252,937 ± 37,234 | 0.528 | 92.0% | 44.0% | 83.4% |
| shadow_price_all | 255,516 ± 41,810 | 0.534 | 91.5% | 44.8% | 82.8% |

#### A.2 Learning Rate Sweep (n=30)

| η | Revenue | Cold Start | Capacity Satisfy | λ Stability |
|---|---------|------------|------------------|-------------|
| 0.001 | 244,617 | 90.9% | 82.8% | 0.009 |
| 0.01 | 243,573 | 91.1% | 83.0% | 0.064 |
| 0.05 | 245,868 | 91.4% | 83.0% | 0.139 |
| 0.1 | 247,039 | 91.2% | 82.9% | 0.163 |
| 0.2 | 249,143 | 91.8% | 82.6% | 0.175 |

#### A.3 Final Lambda Values

| Constraint | λ_init | λ_final | Stability |
|------------|--------|---------|-----------|
| capacity | 0.10 | 0.10 | 0.0000 |
| cold_start | 0.50 | 1.25 | 0.0003 |
| head_cap | 0.10 | 0.42 | 0.0000 |
| whale_spread | 0.20 | 0.55 | 0.2335 |
| frequency | 0.30 | 0.36 | 0.0456 |

### B. 代码文件

- **Shadow Price Allocator**: `scripts/simulator/policies_shadow_price.py`
- **Experiment Script**: `scripts/train_shadow_price.py`
- **Results JSON**: `../results/shadow_price_20260109.json`

### C. 运行环境

```
Date: 2026-01-09
Runtime: ~57 minutes
Simulations: 50 + 30 + 30 + 10 = 120
Seed: 42
```
