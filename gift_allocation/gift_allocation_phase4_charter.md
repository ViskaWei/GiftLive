...
### 4.1 MVP-4.1: Simulator V2 - 金额校准

**问题**: V1 金额分布误差大（P50: 2 vs 7.7, 误差 285%）

**方法**:
1. 混合分布拟合：Lognormal + Pareto (threshold fitting)
2. 分位数匹配校准：目标 P50=2, P90=88, P99=1488
3. 用户财富分布重新参数化
...
| ID | 假设 | 验证 MVP | 判断标准 |
|----|------|----------|----------|
| H4.1 | 分位数匹配可校准金额分布 | 4.1 | P50/P90 误差<30% |
...
| Gate | 条件 | 通过动作 | 失败动作 |
|------|------|----------|----------|
| Gate-4A | Simulator V2 校准误差<30% | → 继续 4.4/4.5 | → 调参 |
