...
| K6 | **Two-Stage适合精排** | gift子集Spearman=0.89 | 召回-精排分工 |
...
| 层 | 当前方法 | 目标方法 | 预期收益 |
|----|---------|---------|---------|
| **召回层** | - | Direct Regression Top-M | 保持Top-1%≥54% |
| **精排层** | - | Two-Stage Stage2 | gift子集排序+14pp |
| **分配层** | Greedy + 软约束 | 影子价格统一框架 | 收益+5%，约束满足 |
...
| Gate-5A 召回精排 | ⏳ 待验证 | Top-1%≥56% 且 NDCG@100 提升 |
| Gate-5B 影子价格 | ⏳ 待验证 | 收益+5% vs Greedy |
...
|----|-------------|-----------|-----------------|---------|
| DG6 | 召回-精排分工能否同时保住Top-1%和提升NDCG？ | 决定估计层架构 | Top-1%≥56% & NDCG提升 | If Y → 采用分工; Else → Direct-only |
| DG7 | 影子价格能否统一处理多约束（容量/冷启动/头部cap）？ | 决定分配框架 | 收益≥+5% & 约束满足 | If Y → 替换Greedy+规则; Else → 保留现状 |
...
| # | 原则 | 初步建议 | 需要验证 |
|---|------|---------|---------|
| P11 | 召回用Direct，精排用Two-Stage | 推荐 | Gate-5A |
| P12 | 多约束用影子价格统一框架 | 推荐 | Gate-5B |
...
| 优先级 | MVP | 名称 | 目标 | Gate |
|--------|-----|------|------|------|
| 🔴 P0 | 5.1 | 召回-精排分工 | Top-1%≥56% & NDCG↑ | Gate-5A |
| 🔴 P0 | 5.2 | 影子价格/供需匹配 | 收益+5%，约束统一 | Gate-5B |
