...
└── Q3: 评估层 - 如何可靠评估分配策略？
    ...
    │   └── Q3.2.2: Simulator V2 能否修复金额分布失真？ → ⏳ 待验证 (MVP-4.1+)
    │       ├── H4.1.1: 离散金额档位能否修复 P50/P90 误差？ → ⏳
    │       ├── H4.1.2: 用户预算约束能否校准 Top-1% Share？ → ⏳
    │       └── H4.1.3: 社交效应(羊群+挤出)能否复现打赏模式？ → ⏳
...
| I17 | V1 Simulator 金额分布严重失真 | P50误差285%，Mean误差111%，Top-1% Share误差62% | 分配层结论可能不可靠，需V2校准 | 问题分析 |
| I18 | V1 缺失关键机制 | 无离散档位/预算约束/社交效应/时序建模 | V2需引入4大机制 | 问题分析 |
