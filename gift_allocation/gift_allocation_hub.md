...
| K2 | ❌ ~~延迟反馈不能简单当负例~~ → **打赏延迟中位数=0，延迟问题不存在** | MVP-1.2: ECE改善=-0.010(变差), 延迟中位数=0s | 关闭延迟校正方向(DG2) |
...
- ~~**信念2**~~：~~延迟反馈导致标签污染是核心风险~~ → **打赏延迟中位数=0，问题不存在** (MVP-1.2: ECE改善-0.010)
...
- ✅ ~~**P0 (MVP-1.2)**~~：已完成，延迟中位数=0s，ECE改善=-0.010(变差)，**DG2 关闭**
- 🟡 **P1 (MVP-1.3)**：多任务学习，用密集信号扶起稀疏打赏
- 🟡 **P1 (MVP-0.3)**：Simulator V1 构建，为分配层验证做准备
...
│   ├── Q1.2: 延迟反馈建模(生存分析)能否提升预测准确性？ → ❌ 简单截断足够
...
| **I8** | **延迟校正无效，简单截断足够** | Chapelle: ECE改善0.003, PR-AUC反降9pp | 打赏中位延迟34.5min，观察窗口已足够覆盖；软标签削弱排序能力 | 无需引入延迟校正复杂性 | MVP-1.2 |
...
| DG2 | ~~延迟校正的增益有多大？~~ | 决定是否引入生存建模 | ✅ **已关闭** ECE改善=-0.010(变差), 延迟中位数=0s | 延迟问题不存在，简单负例处理足够 |
...
| 延迟反馈校正 (Chapelle) | MVP-1.2: ECE改善=-0.010(变差), 延迟中位数=0s | 打赏立即发生（中位数=0s），延迟问题不存在 | 文献假设需在具体数据验证；打赏≠广告CVR |
...
| 2026-01-08 | **MVP-1.2 完成** | ❌ 延迟中位数=0s，ECE改善=-0.010(变差)；**DG2关闭**；延迟问题不存在 |
