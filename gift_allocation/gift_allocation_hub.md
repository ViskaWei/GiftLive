...
| K2 | ❌ ~~延迟反馈不能简单当负例~~ → **审计完成：延迟中位数=0s，礼物立即发生** | MVP-1.2+audit: 86.3%礼物delay=0, pct_late_50=13.3%, ECE改善=-0.010 | **DG2关闭**，延迟不是问题 |
| K3 | ❌ ~~凹收益优于贪心~~ → **模拟环境下Δ=-1.17%，收益不显著但公平性改善** | MVP-2.1: Concave Exp Δ=-1.17%, Δ Gini=-0.018 | 简化为贪心+约束 |
| K6 | ✅ **软约束冷启动同时提升收益(+32%)和成功率(+263%)** | MVP-2.2: 探索发现高潜力新主播 | **采用Greedy+软约束冷启动** |
| K4 | 高稀疏场景(98% Y=0)下，直接回归学会了"谁不送礼" | MVP-1.1-fair 公平对比 | 数据稀疏性影响模型选择 |
| K5 | Two-Stage在NDCG@100更优(+14.2pp)，更适合"精排"而非"粗排" | MVP-1.1-fair: NDCG 35.9% vs 21.7% | 考虑召回-精排分工策略 |

**🦾 现阶段信念 [≤10条，写"所以呢"]**
- **信念1**：~~两段式更优~~ → **Two-Stage输是因为Stage2数据不足+OOD，不是模型设计问题** → 需诊断拆解确认
- **信念2**：❌ ~~延迟反馈导致标签污染~~ → **审计完成：86.3%礼物立即发生，延迟不是问题** → DG2关闭
- **信念3**：单点最优推荐≠全局最优分配 → 需引入凹收益分配层+影子价格机制
- **信念4**：密集代理信号(停留、互动)可扶起稀疏打赏信号 → 多任务学习是关键技术路线
- **信念5**：Two-Stage更适合精排(NDCG@100+14.2pp)，Direct更适合召回 → 考虑分工策略
- **信念6**：离线数据若无截止时间截断，Chapelle本来就可能无效 → 需伪在线实验验证

**👣 下一步最有价值  [≤2条，直接可进 Roadmap Gate]**
- 🔴 **P0 (MVP-1.4)**：Two-Stage诊断拆解，定位问题是Stage2/组合噪声/OOD哪个主因
- 🔴 **P0 (MVP-1.2-audit)**：延迟数据审计，修复样本数不一致+pct_late_*定义问题
- 🟡 **P1 (MVP-1.5)**：Stage2正则化/收缩 + 召回-精排分工策略
- 🟡 **P1 (MVP-1.2-pseudo)**：伪在线截断实验，验证Chapelle在真实延迟场景的价值
...
│   │   ├── H1: Stage2数据量/信息量不足（34k vs 1.87M）→ 🔆 待诊断拆解验证
│   │   ├── H2: p×m乘法在排序上放大误差/引入噪声 → 🔆 待Oracle分解验证
│   │   ├── H3: Stage2的OOD预测问题（只见gift样本）→ 🔆 待诊断验证
│   │   └── H4: 实验口径/特征泄漏可能性 → 🔆 待一致性审计
│   ├── Q1.2: 延迟反馈建模能否提升预测准确性？ → ⚠️ 数据红旗待审计
│   │   ├── 🚩 gift样本数不一致: 72,646 (EDA) vs 77,824 (delay) → Join重复?
│   │   ├── 🚩 pct_late_* 口径矛盾: JSON=68.8% vs 报告=0.7%
│   │   └── 🚩 离线数据无截止截断，Chapelle可能本来就不适用
...
| ~~I8~~ | ⚠️ ~~延迟校正无效~~ **待审计** | 样本数72,646 vs 77,824不一致；pct_late_*口径矛盾 | 可能存在Join重复或字段定义错误 | **暂缓DG2关闭，先做一致性审计** | MVP-1.2 🚩 |
...
| # | 红旗 | 现象 | 可能原因 | 处理方案 |
|---|------|------|----------|----------|
| 🚩1 | **样本数不一致** | EDA gift=72,646 vs Delay gift=77,824 (差5k) | gift→click一对多匹配导致重复 | MVP-1.2-audit 核验 |
| 🚩2 | **pct_late_* 口径矛盾** | JSON pct_late_50=0.688 vs 报告"0.7%" | 字段定义/计算代码可能有bug | 审查代码+统一定义 |
| 🚩3 | **延迟中位数=0 vs Weibull median=35min 并存** | 0点质量高(84%) + 正延迟Weibull | 混合分布，单一Weibull拟合不当 | 用 π·δ(0) + (1-π)·Weibull |
...
| **DG1.1** | **Two-Stage输的主因是什么？(Stage2不足/OOD/乘法噪声)** | 决定是否用召回-精排分工策略 | Oracle分解+诊断拆解 | If Stage2主因 → 考虑分工；Else → 彻底关闭 |
| DG2 | ~~延迟校正的增益有多大？~~ | 决定是否引入生存建模 | ⚠️ **暂缓关闭** | 数据存在不一致红旗，需先审计 |
| **DG2.1** | **延迟数据口径是否正确？(样本数/pct_late_*定义)** | 决定DG2结论是否成立 | 一致性审计通过 | If审计通过 → DG2关闭成立；Else → 重做分析 |
| **DG2.2** | **伪在线截断下Chapelle是否有效？** | 决定生产环境是否需要延迟校正 | 模拟未成熟标签场景 | If ECE改善≥0.02 → 有价值；Else → 关闭 |
...
| 延迟反馈校正 (Chapelle) | ⚠️ 暂缓 | 数据存在红旗🚩 | 需先完成DG2.1审计 | 数据一致性是结论基础 |
| 离线Chapelle (无截断) | ❌ 关闭 | 方法论层面不适用 | 离线完整回放无"未成熟标签" | 需伪在线截断验证真实价值 |
