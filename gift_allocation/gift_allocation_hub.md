...
| **K7** | **分层匹配在当前设置无效** | Gini=0.912 无改善，k=1~5 结果相同 | 需重设鲸鱼阈值/容量 |
...
| **鲸鱼分散** | 超载率 | 0.0% | MVP-5.3 |
| | Gini 改善 | **0** (无变化) | MVP-5.3 |
| | k 值影响 | 无 (k=1~5 相同) | MVP-5.3 |
...
│   ├── Q2.2: 鲸鱼分散能否降低集中度？ → ❌ FAIL (Gini无改善，k=1~5相同)
│   └── Q2.3: 羊群效应/攀比效应？ → ⏳ 待验证
│
├── Q3: 评估层 - 如何可靠评估策略？
...
| **I12** | 分层匹配在低鲸鱼密度下无效 | Gini=0.912 无变化，k=1~5 相同 | 需扩大鲸鱼定义(Top5%)或降容量 | MVP-5.3 |
| **I13** | 容量设置过大未触发拥挤 | 超载率仅 0.28%，k约束未生效 | 需降低容量以验证分层效果 | MVP-5.3 |
...
| Gate-5C 鲸鱼分散 | ❌ FAIL | Gini 无改善，保留统一分配 |
| Gate-5D 风险控制 | ⏳ 待验证 | |
...
| ~~分层匹配(Top1%)~~ | Gini=0.912无变化 | 鲸鱼数量过少，需扩大阈值 |
...
| 优先级 | MVP | 名称 | 目标 | Gate |
|--------|-----|------|------|------|
| ~~🟡 P1~~ | ~~5.3~~ | ~~鲸鱼分散~~ | ~~超载率<10%, Gini↓~~ | ~~Gate-5C~~ ❌ FAIL |
| 🟡 P1 | 5.4 | 风险控制 | CVaR@5% 改善 | Gate-5D |
| 🟢 P2 | 5.5 | 多目标生态调度 | Pareto 前沿 | - |
| 🟢 P2 | 5.3' | 鲸鱼分散重设 | 扩大鲸鱼阈值(Top5%)+降低容量 | - |
...
| 2026-01-20 | MVP-5.3 鲸鱼分散 ❌ FAIL，Gate-5C 关闭 |
